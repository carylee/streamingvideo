...
/**
 * Wysiwyg plugin button implementation for video button plugin
 */
Drupal.wysiwyg.plugins['videobutton'] = {
  /**
  ...
  isNode: function(node) {
    return ($(node).is('img.wysiwyg-videobutton'));
  },
  ...
  invoke: function(data, settings, instanceId) {
    ...
    if (data.format == 'html') {
      ...
      if ($(data.node).is('img.wysiwyg-videobutton')) {
        return;
      }
  ...
  detach: function(content, settings, instanceId) {
    ...
    $.each($('img.wysiwyg-videobutton', $content), function (i, elem) {
      //...
    });
    return $content.html();
  },
